...
module.exports = {
    ...
    bollBand : function(data, section, interval, term) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            // Skip if already calculated
            if (typeof data[i]['bb_' + section] !== 'undefined') continue;

            // Skip if interval data, pushing previous data
            if (i % interval != 0) {
                data[i]['sd_' + section] = data[i-1]['sd_' + section];
                data[i]['bb_' + section] = data[i-1]['bb_' + section];
                _result.push(data[i]);
                continue;
            }
